...
import org.apache.pdfbox.util.MyFontsScanner;
...
final class FileSystemFontProvider extends FontProvider {
    ...
    private List<FSFontInfo> loadDiskCache(List<File> files) {
        ...
        for (File file : files) {
            ...
            LOG.debug("load loadDiskCache font :" + file.getAbsolutePath());
        }
        ...
        if (fileExists) {
            LOG.warn("disk cache file exists");
            ...
            LOG.warn("no disk cache file exists");
        }
        ...
        LOG.debug("loadDiskCache size :" + results.size());
        return results;
    }
